...
int main(int argc, char **argv) {
  ...
  const double k_vertical_p = 3;        // P constant of the vertical PID.
  const double k_roll_p = 20.0;           // P constant of the roll PID.
  const double k_pitch_p = 15.0;          // P constant of the pitch PID.
  
  const double k_roll_i = 0.1;
  const double k_pitch_i = 0.1;
  
  const double k_roll_d = 10;
  const double k_pitch_d = 7;
  
  
  double err_roll = 0;
  double last_err_roll = 0;

  double err_pitch = 0;
  double last_err_pitch = 0;
  
  double pid_roll, pid_pitch;
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    // compute PID
    err_roll += CLAMP(roll, -1.0, 1.0);
    pid_roll = k_roll_p * CLAMP(roll, -1.0, 1.0) + k_roll_i * err_roll + k_roll_d * (last_err_roll - err_roll);
    
    err_pitch += CLAMP(pitch, -1.0, 1.0);
    pid_pitch = k_pitch_p * CLAMP(pitch, -1.0, 1.0) + k_pitch_i * err_pitch + k_pitch_d * (last_err_pitch - err_pitch);
    ...
    const double roll_input = pid_roll + roll_acceleration + roll_disturbance;
    const double pitch_input = pid_pitch - pitch_acceleration + pitch_disturbance;
